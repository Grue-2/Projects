...
public class AI_Trainer {
			...
			if (trial != notLoseCount)
			{
				...
				if (notLoseCount/100.0 > previousBest*.50)
				{
					...
					System.out.println("Currently at " + twoPlaces.format(previousBest) 
						+ "% through the test. (Will go back and forth quite a bit, this is fine.)");
					AI.save();
				}
				return true;
			}
	...
	public static boolean testGameOver(){
		...
		for(int i=0;i<3;i++){
			...
			if(sumRow==3){win(1);if(aiIsX)++notLoseCount;return true;}
			else if(sumRow==-3){win(2);if(!aiIsX)++notLoseCount;return true;}
			else if(sumCol==3){win(1);if(aiIsX)++notLoseCount;return true;}
			else if(sumCol==-3){win(2);if(!aiIsX)++notLoseCount;return true;}
			sumDiag+=state[i][i];
			sumDiag2+=state[2-i][i];
		}
			if(sumDiag==3){win(1);if(aiIsX)++notLoseCount;return true;}
			else if(sumDiag==-3){win(2);if(!aiIsX)++notLoseCount;return true;}
			else if(sumDiag2==3){win(1);if(aiIsX)++notLoseCount;return true;}
			else if(sumDiag2==-3){win(2);if(!aiIsX)++notLoseCount;return true;}
			else if(noneEmpty){win(0);++drawCount;++notLoseCount;return true;}
			else return false;
	}
