...
public class ChatScene {
	private static final String DECRPYTION_COMMAND_PHRASE = "#D:";
	private static final int C_PHRASE_LENGTH = DECRPYTION_COMMAND_PHRASE.length();
	private static final String REQUEST_KEY_COMMAND_PHRASE = "#R:";
	...
	private void sendKeyRequest() {
		...
		Message y = x.sendMessage("#R:").complete();
	...
	private void establishKey(boolean loud) {
		...
		User target = jda.asClient().getFriendsByName(StartScene.target, true).get(0).getUser();
		PrivateChannel x = target.openPrivateChannel().complete();
		Message y = x.sendMessage(REQUEST_KEY_COMMAND_PHRASE).complete();
		if (deleteStuff || !loud) {
			y.editMessage("scrambledmymessageplease").queue();
			y.delete().queue();
		}
	...
	private class MessageDecryptor extends ListenerAdapter {
		...
		public void onPrivateMessageReceived(PrivateMessageReceivedEvent e) {
			...
			if (e.getMessage().getAuthor().getName().equals(StartScene.target)
					&& message.startsWith(DECRPYTION_COMMAND_PHRASE)) {
			...
			else if (e.getMessage().getAuthor().getName().equals(StartScene.target)
					 && message.startsWith(REQUEST_KEY_COMMAND_PHRASE)) 
			{
				establishKey(false);
			}
	...
	private Object input(KeyEvent e) throws InvalidKeyException, UnsupportedEncodingException, InvalidKeySpecException,
		...
		if (e.getCode() == KeyCode.ENTER) {
			if (gotKey) {
				...
				User target = jda.asClient().getFriendsByName(StartScene.target, true).get(0).getUser();
				PrivateChannel x = target.openPrivateChannel().complete();
				Message y = x.sendMessage(DECRPYTION_COMMAND_PHRASE + message).complete();
				if (deleteStuff) {
					y.editMessage("scrambledmymessageplease").queue();
					y.delete().queue();
				}
